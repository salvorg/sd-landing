...
// Адаптивное количество частиц в зависимости от мощности устройства
const getParticleCount = (): number => {
    if (typeof window === 'undefined') return 600;
    const cores = navigator.hardwareConcurrency || 2;
    const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (prefersReduced) return 0;
    if (cores <= 2) return 300;
...
export default function AnimatedLogo() {
    ...
    useEffect(() => {
        ...
        // Если reduced motion — не рисуем вообще
        ...
        class Particle {
            ...
            size: number; type: number; rotation: number;
            ...
            update() {
                ...
                if (distance < config.mouseRadius) {
                    ...
                    const force = -config.mouseRadius / Math.sqrt(distance);
        ...
        const extractLogoPoints = (image: HTMLImageElement, amount: number) => {
            ...
            try {
                ...
                for (let i = 0; i < amount; i++) {
                    points.push(validPixels[Math.floor(Math.random() * validPixels.length)]);
                }
            } catch (e) {
                console.error("Error extracting points:", e);
            }
            // Освобождаем память временного canvas
            tCanv.width = 0;
            tCanv.height = 0;
            return points;
        };
        ...
        const animate = () => {
            // Рисуем ТОЛЬКО если секция видна на экране
            if (isVisible && logicalWidth > 0) {
    ...
    return (
        <canvas
            ...
            style={{ width: '150%', height: '150%', maxWidth: 'none' }}
        />
    );
}
