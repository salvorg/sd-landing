...
const getParticleCount = (): number => {
    ...
    if (prefersReduced || isMobile) return 400;
    if (cores <= 2) return 400;
    if (cores <= 4) return 800;
    return 1200;
};
...
export default function AnimatedMap() {
    ...
    useEffect(() => {
        ...
        class Particle {
            ...
            update() {
                const parallaxX = mouse.nx * CONFIG.parallaxStrength * this.zFactor;
                const parallaxY = mouse.ny * CONFIG.parallaxStrength * this.zFactor;
                const finalTargetX = this.targetX - parallaxX;
                const finalTargetY = this.targetY - parallaxY;
                ...
                if (mouse.x !== undefined && mouse.y !== undefined) {
                    ...
                    if (dist < CONFIG.mouseRadius) {
                        const hoverFactor = 1 - (dist / CONFIG.mouseRadius);
                        targetSize = this.baseSize + (this.baseSize * CONFIG.maxHoverScale * hoverFactor);
                    }
                }
                this.currentSize += (targetSize - this.currentSize) * CONFIG.scaleEase;
            }
        }
        ...
        const initApp = () => {
            ...
            ctx.scale(dpr, dpr);
            particlesArray = [];
            const mapPts = extractMapPoints(particleCount);
            mapPts.forEach(pt => particlesArray.push(new Particle(pt)));
            // Сортируем ОДИН РАЗ при инициализации — не каждый кадр!
            particlesArray.sort((a, b) => a.zFactor - b.zFactor);
        };

        const animate = () => {
            // Рисуем только если секция видна
            if (isVisible) {
        ...
        // Debounce resize — не пересоздаём частицы при каждом пикселе изменения
        let resizeTimer: ReturnType<typeof setTimeout>;
        const handleResize = () => {
            clearTimeout(resizeTimer);
            resizeTimer = setTimeout(initApp, 250);
        };

        // IntersectionObserver — останавливаем анимацию когда не видно
